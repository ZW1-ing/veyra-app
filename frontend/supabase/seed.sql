...
('e9fc7e46-a8a5-4fd4-8ba7-af485013e6fa', '测试工作区', '用于测试的工作区。', 4000, 'gpt-4-turbo-preview', '你是一个助手。', 0.5, true, true, '这些是工作区指令。', false, 'private', 'openai');
...
BEGIN
  ...
  ('e9fc7e46-a8a5-4fd4-8ba7-af485013e6fa', workspace1_id, '对话文件夹 1', '用于存放对话的文件夹', 'chats');

  -- start data for files
  INSERT INTO files (user_id, name, description, file_path, size, tokens, type) VALUES
  ('e9fc7e46-a8a5-4fd4-8ba7-af485013e6fa', '示例文件', '用于测试的文件。', 'https://example.com/file1', 1000000, 250, 'pdf');

  -- start data for file_workspaces
  DECLARE
    file1_id UUID;
  BEGIN
    SELECT id INTO file1_id FROM files WHERE name='示例文件';
  ...
  ('e9fc7e46-a8a5-4fd4-8ba7-af485013e6fa', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'private', TRUE, TRUE, 4000, 'gpt-4-turbo-preview', '示例预设', '示例提示词', 0.5, '示例预设说明。', 'openai');

  -- Get preset id
  DECLARE
    preset1_id UUID;
  BEGIN
    SELECT id INTO preset1_id FROM presets WHERE name='示例预设';
  ...
  ('e9fc7e46-a8a5-4fd4-8ba7-af485013e6fa', '爱因斯坦', '爱因斯坦助手。', 'gpt-4-turbo-preview', '', 'private', 4000, TRUE, TRUE, '你扮演爱因斯坦。', 0.5, 'openai');

  -- Get assistant id
  DECLARE
    assistant1_id UUID;
  BEGIN
    SELECT id INTO assistant1_id FROM assistants WHERE name='爱因斯坦';
  ...
  ('e9fc7e46-a8a5-4fd4-8ba7-af485013e6fa', workspace1_id, '对话 1', 'gpt-4-turbo-preview', '你是一个助手。', 0.5, 4000, TRUE, TRUE, 'openai');

  DECLARE
    folder1_id UUID;
  BEGIN
    SELECT id INTO folder1_id FROM folders WHERE name='对话文件夹 1';
    ...
    ('e9fc7e46-a8a5-4fd4-8ba7-af485013e6fa', workspace1_id, '对话 4', 'gpt-4-turbo-preview', '你是一个助手。', 0.5, 4000, TRUE, TRUE, folder1_id, 'openai');
  END;
